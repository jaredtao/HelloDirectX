...
template <int N, int M, typename T> class GenericMatrix
{
public:
    GenericMatrix();
    ...
    friend GenericMatrix<NN, MM, TT> operator+(
        const GenericMatrix<NN, MM, TT> &m1,
    ...
    friend GenericMatrix<NN, MM, TT> operator*(
        const GenericMatrix<NN, PP, TT> &m1,
        const GenericMatrix<PP, MM, TT> &m2);
    template <int NN, int MM, typename TT>
    friend GenericMatrix<NN, MM, TT>
    operator-(const GenericMatrix<NN, MM, TT> &m);
    template <int NN, int MM, typename TT>
    friend GenericMatrix<NN, MM, TT>
    operator*(const GenericMatrix<NN, MM, TT> &m, TT factor);
    template <int NN, int MM, typename TT>
    friend GenericMatrix<NN, MM, TT>
    operator*(TT factor, const GenericMatrix<NN, MM, TT> &m);
    template <int NN, int MM, typename TT>
    friend GenericMatrix<NN, MM, TT>
    operator/(const GenericMatrix<NN, MM, TT> &m, TT divisor);

private:
    T m[N][M];
    template <int NN, int MM, typename TT> friend class GenericMatrix;
};

template <int N, int M, typename T> GenericMatrix<N, M, T>::GenericMatrix()
{
    setToIdentity();
}
...
template <int N, int M, typename T> void GenericMatrix<N, M, T>::setToIdentity()
{
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < M; ++j)
...
template <int N, int M, typename T> void GenericMatrix<N, M, T>::fill(T value)
{
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < M; ++j)
...
template <int N, int M, typename T>
GenericMatrix<N, M, T> &GenericMatrix<N, M, T>::
                        operator=(const GenericMatrix<N, M, T> &o)
{
    for (int i = 0; i < N; ++i)
    {
        for (int j = 0; j < M; ++j)
...
template <int NN, int MM, typename TT>
GenericMatrix<NN, MM, TT> operator+(
    const GenericMatrix<NN, MM, TT> &m1, const GenericMatrix<NN, MM, TT> &m2)
{
...
template <int NN, int PP, int MM, typename TT>
GenericMatrix<NN, MM, TT> operator*(
    const GenericMatrix<NN, PP, TT> &m1, const GenericMatrix<PP, MM, TT> &m2)
{
    GenericMatrix<NN, MM, TT> result(State::Uninitialized);
    for (int i = 0; i < NN; ++i)
    {
