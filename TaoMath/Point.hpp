...
public:
    ...
    constexpr inline Integer& rx() { return mX; }
    constexpr inline Integer& ry() { return mY; }
    constexpr inline void setX(Integer x) { mX = x; }
    constexpr inline void setY(Integer y) { mY = y; }
    ...
    constexpr inline Point& operator+=(T add)
    {
        mX = static_cast<Integer>(std::round(mX + add));
        mY = static_cast<Integer>(std::round(mY + add));
        return *this;
    }
    constexpr inline Point& operator+=(const Point& o)
    {
        mX += o.mX;
        mY += o.mY;
        return *this;
    ...
    template <class T, typename = std::enable_if<std::is_integral<T>::value || std::is_floating_point<T>::value>::type>
    constexpr inline Point& operator-=(T dec)
    {
    ...
    template <class T, typename = std::enable_if<std::is_integral<T>::value || std::is_floating_point<T>::value>::type>
    constexpr inline Point& operator*=(T factor)
    {
    ...
    constexpr inline Point& operator/=(T divisor)
    {
        mX = static_cast<Integer>(std::round(mX / divisor));
        mY = static_cast<Integer>(std::round(mY / divisor));
        return *this;
    }
    constexpr inline Point& operator/=(Integer divisor)
    {
        assert(divisor != 0);
        mX /= divisor;
        mY /= divisor;
...
constexpr inline const Point operator/(const Point& p1, T divisor)
{
    return Point(static_cast<Integer>(p1.x() / divisor), static_cast<Integer>(p1.x() / divisor));
}
template <class T = Integer, class S = Integer>
constexpr inline const Point operator/(const Point& p1, Integer divisor)
{
    assert(divisor != 0);
...
constexpr inline const Point operator*(const Point& p1, T factor)
{
    return Point(static_cast<Integer>(p1.x() * factor), static_cast<Integer>(p1.y() * factor));
}

template <class T, class S = std::enable_if<std::is_floating_point<T>::value || std::is_integral<T>::value>::type>
constexpr inline const Point operator*(T factor, const Point& p1)
{
    return Point(static_cast<Integer>(p1.x() * factor), static_cast<Integer>(p1.y() * factor));
}
