...
Quaternion::Quaternion(real scalar, const Vector3D &v)
    : mX(v.x()), mY(v.y()), mZ(v.z()), mScalar(scalar)
{
}
Quaternion::Quaternion(const Vector4D &v)
    : mX(v.x()), mY(v.y()), mZ(v.z()), mScalar(v.w())
{
}
...
Quaternion Quaternion::fromRotationMatrix(const Matrix3x3 &rot3x3)
{
    ...
    else
    {
        ...
        const float s
            = 2.0f
              * std::sqrt(rot3x3(i, i) - rot3x3(j, j) - rot3x3(k, k) + 1.0f);
        axis[i] = 0.25f * s;
...
Quaternion Quaternion::fromAxes(
...
void Quaternion::getAxes(
    Vector3D &xAxis, Vector3D &yAxis, Vector3D &zAxis) const
{
...
Quaternion
Quaternion::fromDirection(const Vector3D &direction, const Vector3D &up)
{
    if (direction.isNull())
    {
        return {};
