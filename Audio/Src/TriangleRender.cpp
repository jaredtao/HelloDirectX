...
void TriangleRender::init(int width, int height)
{
...
#endif
    auto infos = AudioEngine::GetRendererDetails();

    m_audioEngine = std::make_unique<AudioEngine>(flags, nullptr, infos.at(1).deviceId.data());
    m_soundEffect = std::make_unique<SoundEffect>(m_audioEngine.get(), L"Alarm.wav");
    auto effect = m_soundEffect->CreateInstance(SoundEffectInstance_Use3D);
    if (effect)
    {
        effect->Play(true);
    }
    ...
    {
        std::cout << "instance effect failed" << std::endl;
    }
...
void TriangleRender::update()
{
    if (!m_audioEngine->Update())
    {
        ...
        {
            OutputDebugStringW(L"audio error");
        }
    }
